...
welcome <-
  one_button_page(div(
    ...
    div(
      h3(strong("Pitch and rhythmic motor behaviors")),
      p("Thanks a lot for your interest in participating in this study. This online part of the experiment will take you about 30 seconds. Please complete it five times separated by at least two hours before you come into the lab. In case you have not already selected an appointment for your lab visit on Sona Systems, make sure to contact ben.engler@clin.au.dk.",
      p("PLEASE NOTE: The experiment works best using the browser Chrome, Firefox or Edge. Safari will most likely not work! If possible, please also use a device with a touchscreen, alternatively a clickpad or mouse, rather than a laptop touchpad. You can not use a keyboard."),
      HTML("<br>"),
      HTML("<p style= 'font-size:14px'> <em>The data you generate is completely anonymous. It will be used for research and teaching purposes and shared with other researchers in a publicly available research data repository. Your participation is completely voluntary and you can leave the experiment at any time by simply closing the browser window. If you have questions re. our use of data collected in this project, you can always contact the research group by emailing Cecilie Møller at cecilie@clin.au.dk. By clicking the button below (I understand. Continue!) you confirm that you are at least 18 years old and you give your consent to participate.</em></p>"),
    )
  )),
...
duplets <- dropdown_page(
  label = "duplets",
  prompt = p(strong("How many times did you complete this online task?")),
  save_answer=TRUE,
  choices = c("Please select", "This was my first time", "This was my second time",	"This was my third time",	"This was my fourth time",	"This was my fifth time",	"I have done it more than five times"),
  # alternative_choice = TRUE,
  ...
  validate = function(answer, ...) {
    if (answer=="Please select")
      "Please let us know how many times you have completed this task, including the current time"
    else TRUE
  },
  on_complete = function(answer, state, ...) {
...
code_1 <- text_input_page(
  ...
  prompt = div(
    ...
    HTML("<br>"),
    p("Please enter a unique personal identifier code as follows: Number of letters in your current street name + last three letters of your mom's first name + the result when you sum up the first 4 numbers of your current phone number (without the country code +45). If you live on Main street, your mom's name is Gertrude, and your phone number is 52 12 34 56, your code would be 10ude10."),
    p("Please make sure to enter this exact same identifier code every time you complete this short online task. We will also ask you to disclose your code to us when you come into the lab, such that we can connect your online and in-lab data.")
...
thanks<-final_page(div(
      HTML("<img src='img/au_logo.png'></img> <img src='img/mib_logo.png'></img>"),
            ...
            p("Don't forget to complete this short task five times separated by at least two hours. We will see you in the lab when you are done! "),
...
elts <- join(
 ...
 make_test(
     elts = elts,
     opt = test_options(title="Pitch and rhythmic motor behaviors, Sep.2023",
